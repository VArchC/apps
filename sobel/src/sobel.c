                  ...
                  bitmap_info_header_t *bitmapInfoHeader)
{
    ...
    FILE *filePtr = stdin;//fopen(filename, "rb");
    if (filePtr == NULL) {
        perror("fopen()");
        return NULL;
    }
              ...
              const pixel_t *data)
{
    ...
    FILE* filePtr = stdout;//fopen(filename, "wb");
    if (filePtr == NULL)
        return true;
...
int main(const int argc, const char ** const argv)
{
    ...
    const pixel_t *in_bitmap_data = load_bmp(NULL, &ih);
    ...
    if (save_bmp(NULL, &ih, out_bitmap_data)) {
